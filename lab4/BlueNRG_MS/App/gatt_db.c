...
#define COPY_SAMPLING_FREQUENCY_CHAR_UUID(uuid_struct) COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0x00,0x01,0x11,0xe1,0xac,0x36,0x00,0x02,0xa5,0xd5,0xc5,0x1b)
...
uint16_t HWServW2STHandle, SamplingFrequencyCharHandle, AccGyroMagCharHandle;
...
extern int16_t sampling_frequency;
...
tBleStatus Add_HWServW2ST_Service(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
    return BLE_STATUS_ERROR;
  /* Fill the Sampling Frequency BLE Characteristc */
    COPY_SAMPLING_FREQUENCY_CHAR_UUID(uuid);
    BLUENRG_memcpy(&char_uuid.Char_UUID_128, uuid, 16);
    ret =  aci_gatt_add_char(HWServW2STHandle, UUID_TYPE_128, char_uuid.Char_UUID_128,
                             1,
                             CHAR_PROP_WRITE,
                             ...
                             16, 0, &SamplingFrequencyCharHandle);
    if (ret != BLE_STATUS_SUCCESS)
      return BLE_STATUS_ERROR;
...
// * @retval tBleStatus Status
// */
tBleStatus Acc_Update(int16_t x, int16_t y, int16_t z)
{
  uint8_t buff[2*3];
  tBleStatus ret;
...
void Read_Request_CB(uint16_t handle)
{
  ...
  {
	 Acc_Update(pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);
  }
...
void Write_Request_CB(uint16_t handle, uint8_t *data, uint8_t length, uint16_t conn_handle)
{
	tBleStatus ret;
	if (handle == SamplingFrequencyCharHandle + 1)
	{
		if (length == 1) { // Assuming the sampling frequency is a single byte
			uint16_t new_frequency = data[0]; // Get the new frequency from the data

			// Update the sampling period variable
			sampling_frequency = new_frequency;
			// Log the new frequency for debugging
			printf("New Sampling Frequency set: %d\n", sampling_frequency);
			ret = aci_gatt_write_response(connection_handle, handle, BLE_STATUS_SUCCESS, 0, 0, NULL);
		}else {
            // If the data length is incorrect, return an error response
            printf("Write Request Error: Incorrect length for Sampling Frequency.\n");
            ret = aci_gatt_write_response(connection_handle, handle, BLE_STATUS_ERROR, BLE_STATUS_INVALID_HANDLE, 0, NULL);
        }
	}
	else {
	        // Handle other write requests if necessary
	        printf("Write Request received for unknown handle: %d\n", handle);
	        ret = aci_gatt_write_response(conn_handle, handle, BLE_STATUS_ERROR, BLE_STATUS_INVALID_HANDLE, 0, NULL);
	    }
    if (ret != BLE_STATUS_SUCCESS)
    {
      PRINTF("aci_gatt_gatt_write_response() failed: 0x%02x\r\n", ret);
    }
